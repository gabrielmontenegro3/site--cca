import { ArrowRight, LayoutGrid, Shield, Building } from 'lucide-react'
      ...
      <div className="mx-auto max-w-[1600px] px-4 sm:px-6 lg:px-8">
        <div className="grid items-center gap-8 lg:grid-cols-[1.2fr_1fr]">
          <div>
            ...
            <p className="mt-3 text-lg italic text-[#1e293b]/80">
              A engenharia como estrutura permanente de apoio à decisão
            </p>
            <p className="mt-4 text-[#1e293b]/90">
            ...
            <ul className="mt-3 space-y-2">
              {[
                { icon: LayoutGrid, label: 'Produção' },
                { icon: Shield, label: 'Garantia' },
                { icon: Building, label: 'Operação do ativo' },
              ].map((item) => (
                <li key={item.label} className="flex items-center gap-3">
                  <span className="flex h-8 w-8 items-center justify-center rounded-[8px] bg-[#137FEC]/10">
                    <item.icon className="h-4 w-4 stroke-[1.5] text-[#137FEC]" />
                  </span>
                  <span className="text-[#1e293b]/90">{item.label}</span>
                </li>
              ))}
            </ul>
            <div className="mt-6 flex flex-nowrap gap-3 md:flex-wrap md:gap-4">
              <a
                href="#metodologia"
              ...
              >
                ...
                <ArrowRight className="h-4 w-4 stroke-[1.5] md:h-5 md:w-5" />
              </a>
              <a
                href="#contato"
